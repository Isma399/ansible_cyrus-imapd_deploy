...
#
# 298s = quasiment 5 minutes (5minutes - 2s)
$MINRESET = 298;
...
if ( $timeSinceUpdated > $MINRESET ) {
  ...
  print DBFILE `find $path/incoming -type f -print | wc -l`;
  print DBFILE `find $path/active -type f -print | wc -l`;
  print DBFILE `find $path/hold -type f -print | wc -l`;
  print DBFILE `find $path/deferred -type f -print | wc -l`;
  close( DBFILE );
}
...
if ( $reqMethod eq "-n" ) {
  ...
  if ( $reqOid eq $baseOid ) {
    ...
    print $baseOid.".11\ninteger\n".$data[12]."\n";
  } elsif ( $reqOid eq $baseOid.".11" ) {
    print $baseOid.".12\ninteger\n".$data[13]."\n";
  } elsif ( $reqOid eq $baseOid.".12" ) {
    print $baseOid.".13\ninteger\n".$data[14]."\n";
  } elsif ( $reqOid eq $baseOid.".13" ) {
    print $baseOid.".14\ninteger\n".$data[15]."\n";
  }
